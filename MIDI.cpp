...
BOOL CMIDI::Create(UINT uResID)
{
	return	Create(MAKEINTRESOURCE(uResID));
}


BOOL CMIDI::Create(LPCTSTR pszResID)
{
	//////////////////////////////////////////////////////////////////
	// load resource
	...
	return Create(pTheSound, dwTheSound);
}
...
MidiJukeBox::~MidiJukeBox()
{
	m_fQuitting = true;
	delete m_pMidi;
	for (int i = 0; i < cpGarbage; i++)
	{
		delete m_ppGarbage[i];
	}
}

void MidiJukeBox::PlayNextSong(bool Auto)
{
	if (m_fQuitting)
		return;
	...
	if (m_pMidi && !Auto)
	{
		m_pMidi->Stop(); // This will trigger next song via CMIDI::OnMidiOutClose()
		return;
	}
	...
	m_iCurrentSong++;
	if (m_iCurrentSong >= (sizeof(Songs) / sizeof(Songs[0])))
		m_iCurrentSong = 0;

	if (Auto)
	{
		CMIDI ** temp = new CMIDI * [1 + cpGarbage];
		for (int i = 0; i < cpGarbage; i++)
			temp[i] = m_ppGarbage[i];
		delete[] m_ppGarbage;
		m_ppGarbage = temp;
		m_ppGarbage[cpGarbage++] = m_pMidi;
	}

	m_pMidi = new CMIDI;
	m_pMidi->Create(Songs[m_iCurrentSong]);
	m_pMidi->Play();
}

void MidiJukeBox::TogglePause()
{
	if (!m_pMidi)
		return;
